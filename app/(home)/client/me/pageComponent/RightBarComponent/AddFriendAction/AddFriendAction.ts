...
export async function AddFriendAction(formData: FormData): Promise<{message: string, color: string, checkIt: boolean}>{
    ...
    if (!allowed) return { message: 'Try again later.' , color: 'red', checkIt: false };


    const FriendLogin = formData.get('FriendLogin')

    if (!FriendLogin) return { message: 'Please enter a username.', color: 'red' , checkIt: false};


    const FriendIdQuery = await pool.query('SELECT id FROM users WHERE login = $1',[FriendLogin]);

    if (FriendIdQuery.rows.length === 0) return { message: 'Hm, didn`t work.Double check that the username is correct.', color: 'red', checkIt: false };
